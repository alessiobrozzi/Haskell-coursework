...
isBloodOrange (Orange(x,_)) | x == "Tarocco" || x == "Moro" || x == "Sanguinello"       = True
                            | otherwise                                                 = False
...
bloodOrangeSegments fruits = length [ (x) | (x) <- fruits, isBloodOrange (x)]

-- 3.
isworm :: Fruit -> Bool
isworm (Orange(_,_))                                             = False
isworm (Apple(_,y))                                              = y

worms :: [Fruit] -> Int
worms fruits = length [ x | x <- fruits, isworm x]
...
eval e (p :<->: q)    =  (eval e (Not p) || eval e q) && (eval e p || eval e (Not q))
...
equivalent p1 p2 = and [(eval x1 p1 == eval x1 p2) && (eval x2 p1 == eval x2 p2) | (x1,x2) <- zip (envs (nub (names p1))) (envs (nub (names p2)))]
...
subformulas T = [T]
            Var x = [Not x]
            Not p = [Not p] ++ subformulas p
            p :&: q = [p :&: q] ++ nub sub p ++ nub sub q
            p :|: q = [p :|: q]
...
toNNF (Not (p :<->: q) = (toNNF p :&: (toNNF (Not q)) :|: (toNNF q :&: (toNNF (Not f)))
